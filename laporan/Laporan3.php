...
$id = $_GET['kode_pemilik'];
$no = 0;
$mahasiswa = mysqli_query($connect, "select * from tbl_pengecekan where no = '$id'");
...
$id = $_GET['no'];
$no = 0;
$mahasiswa = mysqli_query($connect, "select * from tbl_pemeriksaan where no = '$id' " );
while ($row2 = mysqli_fetch_array($mahasiswa)){
    $pdf->Cell(35,6,$row2['hasil'],1,1,'C');  
}
$pdf->Cell(10,3,'',0,1);
$pdf->setFont('Arial','B',12);
$pdf->Cell(300,6,'Riwayat Pemeriksaan',0,1,'L');
// Memberikan space kebawah agar tidak terlalu rapat
$pdf->Cell(10,3,'',0,1);
$pdf->SetFont('Arial','B',10);
$pdf->Cell(8,6,'NO',1,0,'C');
$pdf->Cell(70,6,'TANGGAL Pemeriksaan ',1,0,'C');
$pdf->Cell(80,6,'TANGGAL Pemeriksaan Terakhir',1,1,'C');
include '../asset/koneksi.php';
$id = $_GET['no'];
$no = 0;
$mahasiswa = mysqli_query($connect, "select * from tbl_pemeriksaan where no = '$id' " );
while ($row = mysqli_fetch_array($mahasiswa)){
	$no++;
    $pdf->Cell(8,6,$no,1,0,'C');
    $pdf->Cell(70,6,$row['tgl_pengecekan_awal'],1,0,'C');
    $pdf->Cell(80,6,$row['tgl_pengecekan_akhir'],1,1,'C');
}
$pdf->Ln(20);	
